"""Lec16App1.py Reference for HW 6
List of Dictionaries.
students becomes a list.
Revision of Lec14DictionaryApp2.py
...
Recommended: include comments like below in your documentation
data: stored as a global list - students
each student-details: a dictionary of attributes name, score, grade
student_details
[
...
def process_line(line_in, sep=None): #<---------------------------------'overloading'
    ...
    #append student_details into the global list, students.
...
def submit():
    ...
    print(sid, name, score, grade) #DIY print nicely : add text, format specifiers.


def load():
    """read data from a text file with comma separated values:
    name,graded(1/0),score. fill students dictionary with matching data.
    """
...
def save():
    """save the contents of the global dictionary to a comma seperated text file.
...
def search():
    ...
    for student_details in students: #<-----------------
        ...
        if sid == sid_in:
            ...
            break #id is unique, if I found a match, stop search
    
    if not found_id: #<---------------------------------------
        print(f'No student with id {sid_in:d} in our database.')
...
def summary():
    ...
    #display average score, average for A, count of A.
    average_score, average_for_A, count_A = compute_averages()

    if average_score is not None:
        print(average_score) #format, text
        #......
    else:
        print('No data for average')
    
    print(count_A) #format, text


def reset():
    global students
    
    students.clear() #don't forget the parenthesis!
    # Note: do not use students = {} to reset the dictionary. must use .clear()


#main
#Ignore commented out lines below: just notes.
# slist = [
# {'Sid':1001, 'Name': 'Alex', 'Score':90, 'Grade':'A'},
# {'Sid':1002, 'Name': 'Ben', 'Score':80, 'Grade':'B'},
# ]
# for x in slist:
# print(x) each x is the dict, student_details
# name = x['Name']
# print(name)
...
# print(x) #1001, 1002 the x is sid here.
...
(find the option by searching for
python terminal
or
execute in file dir)
in vs code, always save your text files.
